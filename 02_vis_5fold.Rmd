...
```
## Overall 5-fold model performance

```{r overall performance (5-fold CV), eval=F, echo=F, fig.height=12, fig.width=8}
overall_perfm <- function(target_poll){
   ...
   all_test <- left_join(all_test[[1]], df_all)
   all_test <- lapply(unique(all_test$zoneID), function(id){
      all_test %>% filter(zoneID==id)
   })
   ...
   em_df1_sep <- rbind(data.frame(slr=KGE(all_test_sep[[1]]$slr, all_test_sep[[1]]$obs),
                                  rf=KGE(all_test_sep[[1]]$rf, all_test_sep[[1]]$obs),
                                  EM='KGE', row.names = 'KGE'), em_df1_sep)
   ...
   em_df2 <- rbind(data.frame(slr=KGE(all_test2[[1]]$slr, all_test2[[1]]$obs),
                              rf=KGE(all_test2[[1]]$rf, all_test2[[1]]$obs),
                              EM='KGE', row.names = 'KGE'), em_df2)
   ...
   p3 <- ggplot(em_df %>% filter(EM=='KGE') %>%  gather("model", "values", -c("EM", 'config')), 
          aes(x=model, y=values, fill=config))+
      geom_bar(stat="identity", position = "dodge2")+
      # facet_grid(EM~., scales ='free')+
      # labs(title=years[[i]])+
...
# do.call(grid.arrange, overall_perfm('O3'))
# do.call(grid.arrange, overall_perfm('PM2.5'))
...
annual_perf <- function(target_poll){
   ...
   all_my <- all_m %>% 
      ## Make sure that the predictions have observations available for more than 9 months
      filter(sta_code%in%(names(table(all_m$sta_code)[table(all_m$sta_code)>=9]))) %>% 
   ...
   em_df1 <- rbind(data.frame(slr=KGE(all_my$slr, all_my$obs),
                              rf=KGE(all_my$rf, all_my$obs),
                              EM='KGE', row.names = 'KGE'), em_df1)
   em_df1$config <- '12-month'
   
   # Monthly models (built for each month)
   em_df1_sep <-  show_EM(list(all_m_sepy), 1)
   em_df1_sep <- rbind(data.frame(slr=KGE(all_m_sepy$slr,all_m_sepy$obs),
                              rf=KGE(all_m_sepy$rf, all_m_sepy$obs),
                              EM='KGE', row.names = 'KGE'), em_df1_sep)
   em_df1_sep$config <- '1-month'
   ...
   p3 <- ggplot(em_all %>% filter(EM=='KGE') %>%  gather("model", "values", -c("EM", 'config')), 
          aes(x=model, y=values, fill=config))+
   geom_bar(stat="identity", position = "dodge2")+
   # facet_grid(EM~., scales ='free')+
   # labs(title=years[[i]])+
   labs(y="KGE", title = target_poll)+
   theme(axis.title = element_text(size = 18),
         axis.text = element_text(size = 16),
         legend.title = element_text(size = 16),
         legend.text = element_text(size = 16),
...
* Similar performance between the models for NO2

```{r aggregated annual no2}
do.call(grid.arrange, annual_perf('NO2'))
```

* 12-month SLR model performs the worst. This can be observed in O3, PM10, and PM2.5.
...
annual_perfm_region <- function(target_poll){
   ...
   all_test3 <- all_test2 %>% 
      filter(sta_code%in%(names(table(all_test2$sta_code)[table(all_test2$sta_code)>=9]))) %>% 
      group_by(sta_code, year, climate_zone) %>% 
      summarise(rf=mean(rf), slr=mean(slr), .groups = 'drop') %>% 
      inner_join(., all_test2_yr[[1]][, c('obs','sta_code')], by='sta_code')
   
   em_df1 <- lapply(unique(all_test3$climate_zone), function(id){
      all_test3 %>% filter(climate_zone==id)
   }) %>% lapply(seq_along(.), show_EM, all_test=., region=T)
   ...
   all_test_sep3 <-  all_test_sep2 %>% 
      filter(sta_code%in%(names(table(all_test_sep2$sta_code)[table(all_test_sep2$sta_code)>=9]))) %>% 
      group_by(sta_code, year, climate_zone) %>% 
      summarise(rf=mean(rf), slr=mean(slr), .groups = 'drop') %>% 
      inner_join(., all_test2_yr[[1]][, c('obs','sta_code')], by='sta_code')
   
   em_df1_sep <- lapply(unique(all_test_sep3$climate_zone), function(id){
      all_test_sep3 %>% filter(climate_zone==id)
   }) %>% lapply(seq_along(.), show_EM, all_test=., region=T)
   ...
   # Annual models (bult for a year)
   all_test2_yr[[1]] <- inner_join(all_test2_yr[[1]], df_all,
                                   by='sta_code') %>% 
      filter(sta_code%in%unique(all_test3$sta_code))
   
   em_df2 <- lapply(unique(all_test2_yr[[1]]$climate_zone), function(id){
      all_test2_yr[[1]] %>% filter(climate_zone==id)
   }) %>% lapply(seq_along(.), show_EM, all_test=., region=T)
   ...
   em_df <- rbind(em_df1, em_df1_sep, em_df2)
   heat_df <- em_df %>% filter(EM=='rsq') %>%  gather("model", "values", -c("EM", 'config', 'climate_zone'))
   heat_df$name <- paste0(heat_df$model, '-', 
                          unlist(lapply(strsplit(heat_df$config, '-'), `[[`, 1)))
    pal <- wes_palette("Zissou1", 100, type = "continuous")
   # Heatmap
   p1 <- ggplot(heat_df, aes(x=climate_zone, y=name, fill=values))+
      geom_tile()+
   ...
   # barplot
   p2 <- ggplot(em_df %>% filter(EM=='rsq') %>%  gather("model", "values", -c("EM", 'config', 'climate_zone')),
          aes(x=model, y=values, fill=config))+
      geom_bar(stat="identity", position = "dodge2")+
