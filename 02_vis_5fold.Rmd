...
show_EM <- function(all_test, all_df_i, region=F, month=F){
   ...
   df_all <- data.frame(slr=error_matrix(all_test[[all_df_i]]$obs, slr),
                        ...
                        rf=error_matrix(all_test[[all_df_i]]$obs, rf))[c(1,5,7,8),]
...
overall_perfm <- function(target_poll){
   ...
   all_test_sep <- lapply(all_test_sep, function(test_df){
      test_df[!is.na(test_df$gwr),]
   })
   em_df1_sep <- lapply(seq_along(all_test_sep), show_EM, all_test=all_test_sep)
   em_df1_sep <- lapply(all_test_sep, function(sep_df){
      lapply(unique(sep_df$month), function(target_m){
         show_EM(list(sep_df %>% filter(month==target_m)), 1, F, T)
      }) %>% do.call(rbind, .)
   }) %>% do.call(rbind, .)
...
ggplot(em_df %>% filter(EM=='rsq') %>%  gather("model", "values", -c("EM", 'year','month', 'count', 'date', 'poll')), 
          aes(x=date, y=values, col=model))+
      geom_line()+
      geom_point()+
      labs(y="R squared")+
      ...
      facet_grid(model~poll, scales ='fixed')+
      theme_bw()+
...
ggsave('results/figures/monthly_perfm.png', height=5.5, width=15, units='in',dpi=100)
```
...
# NAs in the gwr predictions:
# em_all[is.na(em_all$gwr), ]
# Aggregate the predictions into annual scale
```


```{r aggregated annual performance (5-fold CV) R2}
ggplot(em_all %>% filter(EM=='rsq') %>%  gather("model", "values", -c("EM", 'config', 'year', 'poll', 'date')), 
   ...
   theme(axis.title = element_text(size = 18),
         ...
         axis.text.x = element_text(angle=90))
ggsave('results/figures/monthly_annual_perfm.png', height=5.5, width=10, units='in',dpi=300)
```
